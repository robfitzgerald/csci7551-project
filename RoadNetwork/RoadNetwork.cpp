...
namespace csci7551_project
{
  ...
  void RoadNetwork::runAllShortestPaths (std::vector<ODPair>* odPairs, std::vector<Path*>* paths)
  {
    ...
    #pragma omp parallel shared(odPairs,intersections,coordinates,distances,search,hasFrontier,stoppingConditionNotMet) private(i)
    {
      ...
      for (i = 0; i < odPairs->size() * 2; ++i)
      {
        ...
        std::cout << "shortestPath jobID " << std::setw(4) << i << ": total distance: " << distance << std::endl;
        Path* result = shortestPath((*odPairs)[i/2], distance, intersections, coordinates, distances, search, i, hasFrontier, stoppingConditionNotMet);
        #pragma omp critical
        if (isLocalMaster(i))
        {
  ...
  Path* RoadNetwork::shortestPath (ODPair od, double dist, std::vector<std::list<Intersection*> > &intersections, std::vector<std::list<std::pair<double,double> > > coordinates, std::vector<std::list<double> > distances, std::vector<BidirectionalAStar*> search, int jobID, std::vector<bool> hasFrontier, std::vector<bool> stoppingConditionNotMet)
  {
    std::cout << "shortestPath function while loop iteration, job " << jobID << std::endl;
